...
CREATE MATERIALIZED VIEW sta."FEATURES" AS 
 SELECT sta.numeric_id_feature(foi.cdprelevement || '-' || lpad(foi.cdstationmesureeauxsurface, 8, '0')) AS "ID",
    ...
    'application/geo+json' AS "ENCODING",
    '{"type":"Point","coordinates":[' || sta.longitude || ',' || sta.latitude || ']}' AS "FEATURE",
    st_geomfromtext('POINT(' || sta.longitude || ' ' || sta.latitude || ')') AS "GEOM",
    sta.foi_prop(foi.finaliteprel::text, rp.code::text, rp.nom::text, foi.dateprel::text, foi.heureprel::text, foi.datefinprel::text, foi.heurefinprel::text, sta.codecourseau::text, sta.libellecourseau::text, cefoi.codeoperationcep::text) AS "PROPERTIES"
   FROM sta.foibase foi
     ...
     left join sta.ce_foibase cefoi on foi.codeoperationcep = cefoi.codeoperationcep
