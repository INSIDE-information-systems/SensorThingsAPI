...
CREATE MATERIALIZED VIEW sta."FEATURES" AS 
 SELECT foi.cdprelevement || '-' || lpad(foi.cdstationmesureeauxsurface, 8, '0') AS "ID",
    ...
    sta.foi_prop(foi.finaliteprel, rp.code, rp.nom, foi.dateprel::text, foi.heureprel::text, foi.datefinprel::text, foi.heurefinprel::text, sta.codecourseau, sta.libellecourseau) AS "PROPERTIES"
